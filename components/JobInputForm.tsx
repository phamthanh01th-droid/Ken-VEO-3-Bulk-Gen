...
export const JobInputForm: React.FC<JobInputFormProps> = ({ onAddJobs, disabled }) => {
  ...
  const resetForm = () => {
    setPrompt('');
    // Do not reset settings, as user might want to add more jobs with same config
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-gray-200">Step 1: Configure Job Settings</h3>
            {isConfigLocked && (
                <button
                    type="button"
                    onClick={() => setIsConfigLocked(false)}
                    disabled={disabled}
                    className="text-sm font-medium text-indigo-400 hover:text-indigo-300 disabled:text-gray-500 transition-colors"
                >
                    Change Settings
                </button>
            )}
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <select id="input-type" value={inputType} onChange={(e) => setInputType(e.target.value as InputType)} disabled={disabled || isConfigLocked} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md text-gray-200 transition-opacity disabled:opacity-70">
            ...
            <div>
              ...
              <select id="model" value={model} onChange={(e) => setModel(e.target.value as Model)} disabled={disabled || isConfigLocked} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md text-gray-200 transition-opacity disabled:opacity-70">
            ...
            <div>
              ...
              <select id="aspect-ratio" value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value as AspectRatio)} disabled={disabled || isConfigLocked} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md text-gray-200 transition-opacity disabled:opacity-70">
            ...
            <div>
              ...
              <input type="number" id="output-count" value={outputCount} onChange={(e) => setOutputCount(Math.max(1, parseInt(e.target.value)))} min="1" max="4" disabled={disabled || isConfigLocked} className="mt-1 block w-full pl-3 pr-2 py-2 text-base bg-gray-700 border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md text-gray-200 transition-opacity disabled:opacity-70"/>
            </div>
        </div>
      </div>

      {/* --- STEP 2: PROMPTS (conditionally rendered) --- */}
      {isConfigLocked ? (
        <div className="space-y-4 pt-6 border-t border-gray-700/50">
          <h3 className="text-lg font-semibold text-gray-200">Step 2: Add Prompts</h3>
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            disabled={disabled}
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-900/50 disabled:text-gray-500 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300 shadow-md"
          >
            Continue to Add Prompts
          </button>
        </div>
      )}
    </form>
